...
export class RewriteAppController {
  ...
  private readonly searchControllers: Record<"label" | "merge", AbortController | null> = {
    label: null,
    merge: null,
  };
  private readonly searchRequestIds: Record<"label" | "merge", number> = {
    label: 0,
    merge: 0,
  };
  ...
  private async performSearch(surface: "label" | "merge", query: string): Promise<void> {
    ...
    const current = surface === "label" ? this.state.labelSearch : this.state.mergeSearch;
    this.patch({
