...
| S6 | **Global state management via 15+ `useState` hooks is fragile** | The legacy component had 17 state variables, multiple refs, and complex interdependencies. The recommendation was to extract the flow into explicit state machines or a reducer so the lifecycle rules documented in `fsm-audit.md` could be enforced in code rather than implicit |
